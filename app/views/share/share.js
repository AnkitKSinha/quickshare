...
var deleteallbutton = new button.Button();
...
var enabled = new observable.Observable();
var imageSource = require("image-source");
var documents = fs.knownFolders.documents();
var page;
var imageName;
var labeltext = new observable.Observable();
mainViewModel.set("uploadedItems", uploadedItems);
mainViewModel.set("labeltext", labeltext);
mainViewModel.set("enabled", enabled);
...
folder.getEntities().then(function(mainentities) {
    console.log(JSON.stringify(mainentities))
    if(mainentities.length == 0){
        labeltext.set("name", "You don't have any files")
        enabled.set("enable", false)
    }
    else{
        labeltext.set("name", "Recent files")
        enabled.set("enable", true)
    }
    ...
    var onQueryEvent = function(result) {
        ...
        if (!result.error) {
            ...
            if(keyNames.includes(receiver)){
                ...
                loader.hide()
                dialogsModule.alert({message: "User does not exist", okButtonText: "OK"});
            }
        }
    };
...
function deleteitem(args) {
    ...
    var index = uploadedItems.indexOf(item);
    ...
    file.remove().then(function(result) {
        ...
        senderfolder.getEntities().then(function(entities) {
            ...
            if (entitylen == 0) {
                console.log("folder")
                var file = folder.getFolder(senderfolder["_name"]);
                file.remove()
    .then(function (result) {
        console.log("removed")
...
folder.getEntities().then(function(mainentities) {
    mainentities.forEach(function(mainentity) {
console.log(JSON.stringify(mainentity))
    })
})
    }, function (error) {
